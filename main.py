...
def run(net, logger, hps):
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.75, patience=4, verbose=True)
    #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 5, gamma=0.5, last_epoch=-1, verbose=True)
